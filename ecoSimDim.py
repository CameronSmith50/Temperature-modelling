...
class dimensionalSim(object):
    ...
    def __init__(self, pars, saveRes = False):
        '''
        Initialisation. pars should be a dictionary generated by initDim().
        saveRes checks if a simulation and its plot should be saved
        '''

        self.pars = pars
        self.rW = pars['growthW']  # Growth of W
        self.rM = pars['growthM']  # Growth of M
        self.beta = pars['transmission']  # Transmission
        self.phiW = pars['fecunW'] # Fecundity loss of W
        self.phiM = pars['fecunM'] # Fecundity loss of M
        self.theta = pars['natMort'] # Natural mortality
        self.sigmaW = pars['virW'] # Virulence of W
        self.sigmaM = pars['virM']  # Virulence of M
        self.q = pars['comp']  # Competition effects
        self.a = pars['pathShed']  # Shedding of pathogen on plate
        self.eta = pars['pathLoss']  # Reduction of pathogen on transmisison
        self.gamma = pars['pathDil'] # Dilution effects
        self.delta = pars['pathDecay']  # Decay of pathogen
        self.rho = pars['pathGrow'] # Initrinsic growth rate of pathogen
        self.K = pars['pathCap'] # "Carrying capacity" of pathogen
        self.saveRes = saveRes  # Do we save the simulation?

        # Initialise the state and times
        self.state0 = np.array([self.q/2, 0.0, self.q/2, 0.0, self.K])
    ...
    def RHSFunction(self, t, state):
        ...
        SW = self.rW*(state[0] + (1-self.phiW)*state[1])*(1-N/self.q) - \
                self.beta*state[0]*state[4] - self.theta*state[0]

        # ODE for infected wildtype
        IW = self.beta*state[0]*state[4] - self.sigmaW*state[1] - \
                self.theta*state[1]

        # ODE for susceptible mutant
        SM = self.rM*(state[2] + (1-self.phiM)*state[3])*(1-N/self.q) - \
                self.beta*state[2]*state[4] - self.theta*state[2]

        # ODE for infected mutant
        IM = self.beta*state[2]*state[4] - self.sigmaM*state[3] - \
                self.theta*state[3]

        # ODE for pathogen
        P = self.rho*state[4]*(1-state[4]/self.K) - self.delta*state[4] + \
                self.a*(state[1]+state[3]) - (self.eta*self.beta*(state[0] + \
    ...
    def plot(self):
        ...
        if self.saveRes:
            ...
            fig.savefig(f'./Figures/{self.saveName}/dimEcoSim.png',
                         bbox_inches='tight')
            fig.savefig(f'./Figures/{self.saveName}/dimEcoSim.pdf',
                         bbox_inches='tight')
        else:
            plt.show()
    ...
    def save(self):
        ...
        saveName = LU.createSaveName(prefix = 'DIM')

        # Add an entry to the lookup table
        LU.addEntrytoLookup(saveName, self.pars, 'DIM')
...
if __name__ == '__main__':
    ...
    # window 
        ...
        sim = dimensionalSim(pars, saveRes = True)
        sim.solveSystem()
        sim.plot()